...
function links_in_same_directory()
{
    ...
    PGFID_FILE1=`getfattr -m "user.pgfid.*" -de hex  $B0/brick/file1 2>&1 | grep "user.pgfid" | gawk -F '=' '{print $2}'`

    # compare the pgfid(link value ) of each  hard links are equal or not
    for i in `seq  2 50`; do
        TEMP=`getfattr -m "user.pgfid.*" -de hex $B0/brick/file$i 2>&1 | grep "user.pgfid" | gawk -F '=' '{print $2}'`
...
function links_across_directories()
{
    ...
    LINES=`getfattr -m "user.pgfid.*" -de hex $B0/brick/dir1/file1 2>&1 | grep "user.pgfid" | wc -l`
    TEST [ $LINES = 2 ]

    for i in $(seq 1  2); do
        HL=`getfattr -m "user.pgfid.*" -de hex $B0/brick/dir$i/file$i 2>&1 | grep "user.pgfid" | awk -v n=$i 'NR==n' | cut -d'=' -f2`
    ...
    HL=`getfattr -m "user.pgfid.*" -de hex $B0/brick/dir1/file1 2>&1 | grep "user.pgfid" | cut -d'=' -f2`
    TEST [ $HL = "0x00000001" ]

    #rename file under same directory

    TEST touch $M0/r_file1
    PGFID_rfile1=`getfattr -m "user.pgfid.*" -de hex $B0/brick/r_file1 2>&1 | grep "user.pgfid"`

    #cross check whether hard link count is one
    HL=`getfattr -m "user.pgfid.*" -de hex $B0/brick/r_file1 2>&1 | grep "user.pgfid" | cut -d'=' -f2`
    ...
    HL=`getfattr -m "user.pgfid.*" -de hex $B0/brick/r_file2 2>&1 | grep "user.pgfid" | cut -d'=' -f2`
    ...
    PGFID_newDir=`getfattr -m "user.pgfid.*" -de hex $B0/brick/dir3/r_file2 2>&1 | grep "user.pgfid"`

    #now the older pgfid and new pgfid shouldn't match
    TEST [ $PGFID_rfile1 != $PGFID_newDir ]

    HL=`getfattr -m "user.pgfid" -de hex $B0/brick/dir3/r_file2 2>&1 | grep "user.pgfid" | cut -d'=' -f2`
    TEST [ $HL = "0x00000001" ]
    ...
    for i in $(seq 1 2); do
            NL=`getfattr -m "user.pgfid" -de hex $B0/brick/dir2/rl_file_$i 2>&1 | grep "user.pgfid"|wc -l `
            TEST_IN_LOOP [ $HL = "0x00000001" ]
    done

    #now pgfid of both files should match
    P_rl_file_1=`getfattr -m "user.pgfid" -de hex $B0/brick/dir2/rl_file_1 2>&1 | grep "user.pgfid"`
    P_rl_file_2=`getfattr -m "user.pgfid" -de hex $B0/brick/dir2/rl_file_2 2>&1 | grep "user.pgfid"`
    TEST [ $P_rl_file_1 = $P_rl_file_2 ]

    #now the no of hard link should be two for both rl_file_1 and rl_file_2
    for i in  $(seq 1 2); do
        HL=`getfattr -m "user.pgfid" -de hex $B0/brick/dir2/rl_file_$i 2>&1 | grep "user.pgfid" | cut -d'=' -f2`
