...
# getfattr of "user.glusterfs.gfidtopath" was succeeding for that
...
# and the user.glusterfs.gfidtopath xattr should give the
# path of the object as the value

TEST getfattr -n user.glusterfs.gfidtopath $M0/dir/file
...
# trying the getfattr. Because, user.glusterfs.gfidtopath xattr
# request is handled only if gfid2path is enabled. If not,
# then getxattr on user.glusterfs.gfid2path fails anyways. In this
...
TEST ! getfattr -n user.glusterfs.gfidtopath $M0/new/foo;
...
TEST getfattr -n user.glusterfs.gfidtopath $M0/3/new
